...
export async function POST(request: NextRequest) {
  try {
    ...
    const lambdaResponse = await fetch(process.env.AWS_LAMBDA_PYTHON_URL || 'https://your-lambda-url.amazonaws.com/runPythonRules', {
      method: 'POST',
      headers: {
    ...
    if (!lambdaResponse.ok) {
      throw new Error(`Lambda function failed with status ${lambdaResponse.status}`);
    }
